part of app_constants;

class LottiePath {
  static const _folderPath = 'assets/lotties';

  static const security = '$_folderPath/security.json';
  static const security2 = '$_folderPath/security_2.json';
  static const registration = '$_folderPath/registration.json';
  static const error404 = '$_folderPath/404.json';
  static const bookLoading = '$_folderPath/book_loading.json';
  static const academic = '$_folderPath/academic.json';
  static const checkMark = '$_folderPath/check_mark.json';
  static const quizMode = '$_folderPath/quiz_mode.json';


}

class ImageRasterPath {
  static const _folderPath = 'assets/images/raster';

  static const googleLogo = '$_folderPath/google-logo.png';
  static const introductionBg = '$_folderPath/introduction_bg.png';
  static const bubble = '$_folderPath/bubble.png';
  static const findAnswer = '$_folderPath/find_answer.png';
}

class ImageVectorPath {
  static const _folderPath = 'assets/images/vector';
}